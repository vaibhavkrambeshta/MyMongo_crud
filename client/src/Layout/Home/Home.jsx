...
const  Home =  () => {
  ...
  const removeEmployee = async id => {
    ...
    try {
      const employeeRemoved = await axios.delete(`http://localhost:5000/api/v1/employees/${id}`, config);
  ...
  const searchEmployees = async key => {
    ...
    try {
        const searchEmployees = await axios(`http://localhost:5000/api/v1/employees/search/${key}`, config);
